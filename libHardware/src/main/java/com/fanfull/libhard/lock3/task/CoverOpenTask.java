...
public abstract class CoverOpenTask extends ReadLockTask {
  ...
  @Override protected void onSuccess(Lock3Bean lock3Bean) {
    ...
    if (lock3Bean.getStatusCheck() != Lock3Util.FLAG_CHECK_STATUS_REG) {
      // 其他业务，标志位不等于 3
  ...
  public boolean onUpdateFailed(int res, Lock3Bean lock3Bean) {
    String msg;
    switch (res) {
    ...
    ThreadUtil.execute(() -> {
      ...
      if (taskType == TASK_TYPE_COVER) {
        if (lock3BeanWrite.pieceEpcBuff == null || lock3BeanWrite.pieceTidBuff == null) {
          ...
          onUpdateFailed(WRITE_RES_ARGS_WRONG, lock3BeanWrite);
          return;
        }
        res = UhfController.getInstance().writeEpcFilterTid(
            lock3BeanWrite.pieceEpcBuff, lock3BeanWrite.pieceTidBuff);
        if (!res) {
          onUpdateFailed(WRITE_RES_WRITE_EPC_FAILED, lock3BeanWrite);
          return;
        }
      }
      ...
      if (taskType == TASK_TYPE_COVER || taskType == TASK_TYPE_OPEN) {
        ...
        for (Lock3InfoUnit infoUnit : willReadList) {
          ...
          onUpdateFailed(WRITE_RES_WRITE_NFC_FAILED, lock3BeanWrite);
          return;
        }
      }
      ...
      if (lock3BeanWrite.handoverBean != null) {
        ...
        if (!res) {
          onUpdateFailed(WRITE_RES_WRITE_HANDOVER_FAILED, lock3BeanWrite);
        }
      }
      onUpdateSuccess(lock3BeanWrite);
    });
  }
  ...
  public Lock3Bean genLock3Bean2Write(
    ...
    } else if (taskType == CoverOpenTask.TASK_TYPE_OPEN) {
      // ====== 开袋业务 ====== 更新 标志位为F4
      int keyNum = lock3Bean.getKeyNum();
  ...
  public abstract boolean onUpdateSuccess(Lock3Bean lock3Bean);
}
