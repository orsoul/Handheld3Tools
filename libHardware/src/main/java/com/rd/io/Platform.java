...
public class Platform {
  ...
  private static int STATE_HIGH = 1;
  private static int STATE_LOW = 0;

  public static void turnLight(boolean turnOn) {
    if (turnOn) {
      Platform.SetGpioDataHigh(9);
    } else {
      Platform.SetGpioDataLow(9);
    }
  }

  public static void turnLight() {
    int state = GetGpioState(9);
    LogUtils.d("%s", state);
    turnLight(state == STATE_LOW);
  }
  ...
  public static void openGPIO() {
    ...
    Platform.SetGpioMode(59, 1);            //MODE1
    Platform.SetGpioMode(60, 1);            //MODE1
    //232 的电源使能脚设置成IO模式
    Platform.SetGpioMode(34, 0);        //RDN1_A    GPIO34    RDP1_A   GPIO35
    //232的GPIO设置成输出
    Platform.SetGpioOutput(34);                //
    //232的电源使能脚设置成输出
    Platform.SetGpioDataHigh(34);
  }
}
